...
| OnionOS bundle | **complete and verified on the dev box, never on a device** — `-DWREEL_TARGET_FIRMWARE=onion` stages `App/Coppers/` with binary, five assets and a vendored SDL2 runtime, and tars it to 13 MB. The link closure is satisfied: the binary needs only `libSDL2-2.0.so.0` from `lib/` plus libc/libm/loader from the device. The runtime is vendored rather than borrowed from the firmware's `parasyte` copy, whose EGL is Mesa and drags in X11, gbm and libdrm — see the snapshot's decision 3. A bundle-shaped tree launched from `/` logs `assets: … (beside the executable)`, writes its log to `.local/share/wreel/coppers/` inside the bundle, and produces the expected 1,228,922-byte screenshot. `launch.sh` is `sh -n` and shellcheck clean. The layout came from real OnionUI packages; **whether Onion's MainUI launches it is unverified** |
| SDL2 on `miyoomini` | **built and link-verified 2026-07-27, never run on a device** — upstream SDL2 cannot drive the panel, so `WREEL_USE_SYSTEM_SDL2=ON` against an `mmiyoo` SDL2 is mandatory. `coppers` now builds that way under GCC 8.3 with zero warnings, links `libSDL2-2.0.so.0` dynamically, keeps SDL2_image/ttf/mixer static, and holds its `GLIBC_2.28` floor. **All 160 SDL symbols it imports resolve** against both the device's copy and steward-fu's prebuilt — checked with `nm -D` against their dynamic symbol tables. Needed a toolchain fix to be possible at all: `WREEL_SDL2_ROOT`, because `CMAKE_FIND_ROOT_PATH_MODE_PACKAGE ONLY` confines `find_package` to a sysroot that has SDL **1.2** and no SDL2 |
